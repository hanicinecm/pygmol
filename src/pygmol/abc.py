...
class PlasmaParameters(ABC):
    ...
    Time-dependent power is encoded by two same-length sequences:
    `power` and `t_power'. An example of a single 500W pulse for half of
    the simulation time of 0.2 seconds might look like
    ``power = [500, 500, 0, 0]`` with ``t_power = [0.0, 0.1, 0.1, 0.2]``
...
class Equations(ABC):
    ...
    The `GlobalModel` (as the highest-level class of the `pygmol`
    package) solves an initial value problem for a system of ordinary
    differential equations. The system being solved (in general) is::
    ...
    Here, t is an independent dime variable, and y(t) is an N-D
    vector-valued function (describing the state as a function of time).
    The N-D vector-valued function f(t, y) describes the right-hand
    side (RHS) of the ODE system and determines the system.

    The concrete `Equations` subclasses need to implement several
    attributes / properties:
    `ode_system_unknowns` attribute is expected to give
    a sequence of unique strings identifying the unknowns *y* in the
    system. This needs to be implemented so the `GlobalModel` downstream
    knows what it is solving for and can create human-readable
    solutions.
    `ode_system_rhs` attribute / property is expected to give the
    function *f(t, y)*, or the RHS of the ODE system. This function
    signature must be ``f(t, y)``, where `t` is a scalar time and `y`
    is an N-D state vector of floats (consistent with the unknowns
    names).
    ...
    Notes
    -----
    Normally, the concrete `Equations` subclass will accept the
    `PlasmaParameters` and `Chemistry` instances in its initializer,
    in order to build the RHS function `ode_system_rhs` and
    `ode_system_unknowns`. This is, however, not formalised by this
    ABC's definition.
    """
    ...
    def ode_system_unknowns(self) -> Sequence[str]:
        """The list of human-readable labels of the state variables
        which are being resolved and solved for by the ODE system.
        This will be used as column names in the solution generated by
        the `GlobalModel` class.

        This is most commonly going to be an instance attribute of any
        concrete subclass of `Equations`, however, to instantiate any
        subclass, a class attribute of the same name will be needed.
        It is advised to define
        ``ConcreteEquations.ode_system_unknowns = None`` as a stub and
        as a reminder that the real unknowns need to be provided as an
        instance attributes (based on the chemistry set and plasma
        parameters).

        Example might look like ["n_e", "n_Ar", "n_Ar+", "n_Ar*", "T_e"]
        for a system resolving densities of heavy species (Ar, Ar+, Ar*)
        and electron temperature and density, or
        ["e", "Ar", "Ar+", "T_e", "T_Ar", "T_Ar+"] for a system
        resolving the densities of e-, Ar, Ar+ and their individual
        temperatures.
        """
    ...
    def ode_system_rhs(self) -> Callable[[float, ndarray], ndarray]:
        """A callable returning the time derivative of the state
        (unknowns being solved for) as a function of time and the
        current state.

        Apart from ``t`` and ``y``, the function dy / dt will also
        depend on other *things* (such as the chemistry set and the
        plasma conditions). The appropriate objects modeling those will
        normally be passed to the concrete `Equations` subclass and
        this dependence will be coded into the concrete
        ``ode_system_rhs`` property.
        """
